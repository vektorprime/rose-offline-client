...
pub struct LuaVMContext<'a, 'w1, 's1, 'w2, 's2> {
    pub function_context: &'a mut crate::scripting::ScriptFunctionContext<'w1, 's1>,
...
pub fn conversation_dialog_system(
    ...
    // Process incoming events to open dialogs
    for event in conversation_dialog_events.read() {
        ...
        *current_dialog_state = None;
            ...
            })
        {
            ...
            if !check_open_function.is_empty() {
                ...
                // No check function, just open the dialog
                if next_dialog_state.generated_dialog.run_menu(
                    &mut next_dialog_state.lua_vm,
                    &mut user_context,
                    &next_dialog_state.con_file,
    ...
    // Render the dialog if one is active
                ...
                dialog.draw(
                    ...
                    |ui, _bindings| {
                        ui.put(
                            ...
                            egui::Label::new(
                                egui::RichText::new(title).color(egui::Color32::WHITE).font(
                                    egui::FontId::new(
                                        14.0,
