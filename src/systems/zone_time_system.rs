...
pub fn zone_time_system(
    ...
    if should_log {
        ...
        log::info!(
            "[ZONE_TIME]     is_night: {} (hours >= 19.0 || hours < 6.0)",
            is_night
        );
        log::info!(
            "[ZONE_TIME]     is_evening: {} (hours >= 17.0 && hours < 19.0)",
            is_evening
        );
        log::info!(
            "[ZONE_TIME]     is_day: {} (hours >= 12.0 && hours < 17.0)",
            is_day
        );
        log::info!(
            "[ZONE_TIME]     is_morning: {} (hours >= 6.0 && hours < 12.0)",
            is_morning
        );
        log::info!("[ZONE_TIME] ======================================");
    }
    ...
    if is_night {
        ...
        if zone_time.state_percent_complete < 0.5 {
            ...
            zone_lighting.volumetric_density_factor = VOLUMETRIC_DAY_DENSITY.lerp(
            ...
            zone_lighting.volumetric_density_factor = VOLUMETRIC_EVENING_DENSITY.lerp(
        ...
        if zone_time.state_percent_complete < 0.5 {
            ...
            zone_lighting.volumetric_density_factor = VOLUMETRIC_NIGHT_DENSITY.lerp(
            ...
            zone_lighting.volumetric_density_factor = VOLUMETRIC_MORNING_DENSITY.lerp(
        ...
        let (temperature, saturation) = match zone_time.state {
            ZoneTimeState::Morning => {
                ...
                if t < 0.5 {
                    ...
                    (
                        COLOR_GRADING_MORNING_TEMPERATURE
                            .lerp(COLOR_GRADING_DAY_TEMPERATURE, lerp_t),
                        COLOR_GRADING_MORNING_SATURATION.lerp(COLOR_GRADING_DAY_SATURATION, lerp_t),
                    )
                }
            }
            ZoneTimeState::Day => (COLOR_GRADING_DAY_TEMPERATURE, COLOR_GRADING_DAY_SATURATION),
                ...
                if t < 0.5 {
                    ...
                    (
                        COLOR_GRADING_DAY_TEMPERATURE
                            .lerp(COLOR_GRADING_EVENING_TEMPERATURE, lerp_t),
                    ...
                    let lerp_t = (t - 0.5) * 2.0;
                    (
                        COLOR_GRADING_EVENING_TEMPERATURE
                            .lerp(COLOR_GRADING_NIGHT_TEMPERATURE, lerp_t),
                        COLOR_GRADING_EVENING_SATURATION
                            .lerp(COLOR_GRADING_NIGHT_SATURATION, lerp_t),
                    )
                }
            }
            ZoneTimeState::Night => (
