use bevy::{asset::Assets, pbr::MeshMaterial3d, prelude::*};
...
use crate::components::{
    GrassBlade, PlayerCharacter, Season, SeasonMarker, SummerFlower, TerrainMeshForGrass,
};
...
pub fn summer_vegetation_system(
    ...
    *frame_counter += 1;
    ...
    let zone_data = current_zone
        .as_ref()
        .and_then(|cz| zone_loader_assets.get(&cz.handle));
    ...
    if current_grass_count < summer_settings.max_grass_blades {
        ...
        let grass_to_spawn =
            ((summer_settings.max_grass_blades - current_grass_count) as f32 * dt * 0.5).min(10.0)
                as usize;
...
fn spawn_grass_blade(
    ...
    let position = Vec3::new(world_x, terrain_height, world_z);

    // Random grass height within range
    let height = summer_settings.grass_height_range.0
        + rand::random::<f32>()
            * (summer_settings.grass_height_range.1 - summer_settings.grass_height_range.0);
...
fn spawn_summer_flower(
    ...
    let position = Vec3::new(world_x, terrain_height + 0.05, world_z);
    ...
    let stem_height = summer_settings.flower_stem_height_range.0
        + rand::random::<f32>()
            * (summer_settings.flower_stem_height_range.1
                - summer_settings.flower_stem_height_range.0);
...
pub fn vegetation_sway_system(
    ...
    mut grass_query: Query<
        (&mut Transform, &GrassBlade),
        (With<GrassBlade>, Without<SummerFlower>),
    >,
    mut flower_query: Query<
        (&mut Transform, &SummerFlower),
        (With<SummerFlower>, Without<GrassBlade>),
    >,
    ...
    for (mut transform, flower) in flower_query.iter_mut() {
        ...
        if to_camera.length_squared() > 0.001 {
            ...
            let sway_rotation = Quat::from_euler(EulerRot::XYZ, sway_z, 0.0, sway_x);
