...
use crate::components::{
    AngelicWings, CharacterModel, CharacterModelPart, FlightState, PlayerCharacter, WingSide,
};
...
impl Plugin for WingSpawnPlugin {
    fn build(&self, app: &mut App) {
        app
            ...
            // Add wing systems
            .add_systems(Update, (wing_spawn_system, wing_animation_system).chain());
...
fn spawn_wings(
    ...
    let left_wing = commands
        .spawn((
            ...
            Transform::from_xyz(-0.1, 0.9, -0.15).looking_at(Vec3::new(-1.5, 0.5, -0.15), Vec3::Y),
...
fn create_angel_wing_mesh(meshes: &mut ResMut<Assets<Mesh>>, side: WingSide) -> Handle<Mesh> {
    ...
    for i in 0..=segments {
        ...
        let base_y = wing_height * (1.0 - t * 0.6) * (1.0 - t * 0.25) * (1.0 + 0.35 * curve_factor);
    ...
    for i in 0..secondary_feather_count {
        ...
        indices.extend_from_slice(&[idx, idx + 2, idx + 3, idx + 1, idx + 3, idx + 2]);
    }
