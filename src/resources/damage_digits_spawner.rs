...
pub struct DamageDigitsSpawner {
   ...
   impl DamageDigitsSpawner {
        pub fn load(
            ...
            log::info!("[DAMAGE_DIGITS_SPAWNER::load] Loaded texture_damage: {:?}", texture_damage);
            
            let texture_damage_player = asset_server.load("3ddata/effect/special/digitnumber02.dds");
            log::info!("[DAMAGE_DIGITS_SPAWNER::load] Loaded texture_damage_player: {:?}", texture_damage_player);
            
            let texture_miss = asset_server.load("3ddata/effect/special/digitnumbermiss.dds");
            log::info!("[DAMAGE_DIGITS_SPAWNER::load] Loaded texture_miss: {:?}", texture_miss);
            
            let motion = asset_server.load("3ddata/effect/special/hit_figure_01.zmo");
            log::info!("[DAMAGE_DIGITS_SPAWNER::load] Loaded motion (ZMO): {:?}", motion);
   ...
   pub fn spawn(
        ...
        log::info!("[DAMAGE_DIGITS_SPAWNER] spawn() called with damage={}, is_damage_player={}", damage, is_damage_player);
        
        let (scale, _, translation) = global_transform.to_scale_rotation_translation();
        log::info!("[DAMAGE_DIGITS_SPAWNER] GlobalTransform: scale={:?}, translation={:?}", scale, translation);

        // Get the appropriate texture
        let texture_handle = self.get_texture(damage, is_damage_player);
        log::info!("[DAMAGE_DIGITS_SPAWNER] Selected texture handle: {:?}", texture_handle);
        ...
        let child_entity = commands
            ...
            .id();
        log::info!("[DAMAGE_DIGITS_SPAWNER] Child entity spawned with id: {:?}", child_entity);

        // Then spawn the parent and add the child
        let parent_position = translation + Vec3::new(0.0, model_height * scale.y, 0.0);
        log::info!("[DAMAGE_DIGITS_SPAWNER] Parent position: {:?}", parent_position);
        commands
            .spawn((
