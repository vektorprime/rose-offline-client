...
use bevy::{
    asset::{Asset, AssetLoader, BoxedFuture, LoadContext, LoadedAsset, VisitAssetDependencies},
    io::Reader,
    math::{Quat, Vec3},
...
impl VisitAssetDependencies for ZmoAsset {
    fn visit_dependencies(&self, visit: &mut impl FnMut(bevy::asset::AssetPath)) {
        if let Some(ref animation_texture) = self.animation_texture {
            visit(animation_texture.texture.clone().into());
        }
    }
}
