...
Independent sub-agent scrutiny of every finding (F1–F16) against the actual source, egui 0.33, Bevy 0.18.1, the shared data crates, the server quest path, and the previous implementation attempt on `wip/local-changes-2026-08-04` (read-audited in `12-validation.md` — **note: that validation ran no build; the wip VM rewrite was never compiled**). Verdicts per finding:

| Finding | Verdict | Scrutiny result / action |
|---|---|---|
| F1 | CONFIRMED (fix safe — **Arc, not Rc**) | Every allocation site verified (9 read sites + param setup; each String clone = heap alloc + memcpy; table clones recurse the HashMap/Vec). The fix is safe: strings have zero mutation sites; table deep-copy semantics untouched (no aliasing possible — every clone is still a deep copy); TryFrom blast radius is exactly the 3 files named; float formatting untouched. **Mandatory change: use `Arc<str>`, NOT `Rc<str>`** — `Lua4Value` flows through `SystemFuncEvent` (a Bevy `Message`, requires `Send + Sync + 'static`); Rc wouldn't compile. The ~80% figure is directionally right but unmeasured. Implement F1+F2 together (wip's form: `HashMap<Arc<str>>` table keys also kills SETTABLE/SETGLOBAL key clones); add a small regression harness (zero tests exist in src/scripting). |
| F2 | CONFIRM (already on wip) | Per-literal-execution clone verified. **Already implemented on wip** (`constant_strings: Vec<Arc<str>>`; the OP_PUSHSTRING `.clone()` becomes a refcount bump with zero code change) — zero semantic change. Port as one unit with F1 (same files; entangled with the F3 rewrite). |
| F3 | CONFIRM (fix on wip, sound — merge) | All claims verified (split_off alloc + tail memcpy per call; arg re-clone; TAILCALL recursive, not a tail call; fresh Vec per call; no depth limit — a few thousand nested frames overflow the 1 MB main-thread stack; exploitability in shipped data unproven — no .con files available to inspect). The wip implementation (`run_function(&mut stack, base, depth)` shared stack + `MAX_CALL_DEPTH = 512` + `CallDepthExceeded`) is behavior-equivalent (verified: the drain/truncate/pad logic reproduces the old reverse/pop result-set semantics exactly). Real tail calls correctly NOT implemented (defer). One nit: the dialog-open path swallows `CallDepthExceeded` via `.ok()?` — consider logging it. Merge wip. |
| F4 | CONFIRMED (fix safe — upvalues are dead data) | Claim verified — and stronger: the captured upvalue Vec is **never read after creation** (all destructure sites ignore it; OP_PUSHUPVALUE reads the callee's local stack; no OP_SETUPVALUE/OP_GETUPVALUE opcodes exist) — the risk note's "possibly non-functional" is definitively non-functional. The `Arc<Vec<Lua4Value>>` fix is 100% behavior-preserving (never mutated, never read, per-OP_CLOSURE fresh allocation). Already on wip; comes with the F1 bundle. Document (don't change) the dead-upvalue fact so nobody "fixes" it silently. |
| F5 | PARTIAL (fix on wip; doc title error) | "Double hash" is **REFUTED** — exactly one hash (table_get destructures the enum and passes `&String`; `Lua4Value` has no Hash impl). **Omission: OP_PUSHSELF does the identical clone+wrap+lookup** (a third site). The fix is F1-INDEPENDENT (works via `&String → &str` deref). Already on wip (`table_get_field(&Lua4Value, &str)` at all three sites). Merge; correct the title to "1 alloc + 1 hash". |
| F6 | CONFIRM (already on wip — merge) | Claim accurate on main (full re-read + XOR decode + parse + VM rebuild + 46 name clones + full bytecode decode per dialog open). **Already implemented on wip** (`CachedConScript { con_file, lua_function }` keyed by `VfsPathBuf` with negative caching; 12-validation:91 OK). Safety verified: Lua4Function immutable and shareable (all state is per-VM — no global leakage across opens); ConFile never mutated; keys normalized; memory bounded (NPC-db conversation_files map); data immutability holds. Nothing to port ahead of the merge. |
| F7 | PARTIAL (2-line tweak bundled with F1) | GETGLOBAL claim accurate (borrow, one hash, no alloc — also holds on wip). SETGLOBAL claim accurate on main — **but the wip's "both become refcount ops" is REFUTED for SETGLOBAL**: `set_global` takes `&str` and does `name.into()` → a NEW `Arc<str>` allocation per write (identical to main). Fix: change `set_global` to take `&Arc<str>` (or clone the Arc in OP_SETGLOBAL) → zero allocs. The doc's "not worth pre-binding slots" rating stands. |
| F8 | CONFIRM (implement-with-changes) | Two-hop claim accurate (cosmetic: the cited lib.rs lines are the effect DB — quest DB loads at lib.rs:1858-1861; immutability claim holds). Fix adjustments: (1) store `HashMap<QuestTriggerHash, QuestTrigger>` directly in `rose-data` — the server touches only method signatures, which are preserved (reimplement `get_trigger_by_name` via hash), so it compiles unchanged; (2) **precomputing `next_trigger_hash` on QsdTrigger is NOT possible** — `rose-file-readers` cannot depend on `rose-data` (layering) — use runtime `QuestTriggerHash::from(name)` per chain step, or a parallel hash→next-hash map in the rose-data-irose load code. Verify the rose-offline server build after the shared-crate change. |
| F9 | PARTIAL (skip — agrees with doc) | Accurate (cheap match decodes; event-driven; None fallbacks) with one overstatement: the coupling claim is partial (clan_position + rewards keep `game_data` regardless). Citation slip (`decode_item_reference` is at :550-557). A parallel decoded struct touches shared crates and changes the failure surface (malformed data: silent-fail today → load-time policy needed). **Skip**; if ever revisited, only log decode failures at quest-DB load time. |
| F10 | PARTIAL (reject/defer; doc corrections) | O(N) scan confirmed — N includes monsters (a few hundred worst case ≈ 1–3 µs). Doc errors: "pure waste" is wrong (the loop IS the lookup; the original C++ client did O(1) via the object index — this is a regression vs the original design, but the fix's cost exceeds the gain); "callable from Lua conditions" is wrong (no Lua condition path exists — only Lua dialog scripts). **Both fix variants flawed**: `HashMap<NpcId, usize>` uses the wrong key (NpcId is a TYPE id — duplicate ids collapse and change first-match semantics; lifecycle drift across 8 spawn/despawn sites); a filtered query is exactly the current loop. If ever done: a dialog-scoped map (one O(N) pass per dialog open). |
| F11 | PARTIAL (defer; hygiene-only merge of the maps) | "Two hashes per call" is worst-case only (the else-if short-circuits: 14 of 18 closures are QF_* → one lookup); the Vec alloc is accurate; on wip the rust boundary still splits off args. Map-merge = 15-line safe hygiene win (names disjoint, verified). The slice/results-buffer redesign is entangled with F3 and churns the public trait + 18 signatures — reject as perf work at event-driven frequency. |
| F12 | PARTIAL (implement-with-changes, downgraded to Low) | Mechanism true (per-frame block while a dialog is open; text changes ONLY via run_menu — verified; invalidation points exhaustive). **Impact overstated**: egui 0.33's GalleyCache retains previous-frame entries — identical jobs are never re-shaped; the real cost is 1–3 µs/frame (N+1 LayoutJob clones + content hashes). **Required addition: a `pixels_per_point` guard** (rebuild when `galleys_dirty || ppp != cached_ppp`) — the doc's sketch as written serves stale geometry on DPI changes. Same pattern applies to the quest-scroll dialog (06-F15) and message box. Not on wip. |
| F13 | PARTIAL (defer; fix as written is half-unsafe) | Mechanics confirmed (1 Arc alloc/frame; single collision player; intersect not throttled — the 5 s gate only guards the write). **Impact overstated**: rapier `intersect_shape` is BVH-pruned — zone-wide object counts are irrelevant (k = 0–2 colliders); real cost 1–3 µs (Low, not Medium). Fix details: `Local<Option<Collider>>` + first-frame init (a bare `Local<Collider>` silently becomes `ball(0.5)` — halves the trigger sphere); full-3D position delta. **The doc's literal skip condition ("delta < 10 cm OR dt < 100 ms") is UNSAFE** — standing still in a trigger zone never re-tests → the 5 s re-fire (which is the retry mechanism for conditions that flip while idle) stops → quests never fire until the player moves. Correct gate: skip only when delta < 10 cm **AND** dt < 100 ms. Defer until collision_system's §3.1/§3.2 repairs land (file mid-repair). |
| F14 | CONFIRM (wire it up — a missing local-state sync, not just dead code) | All claims verified (dispatch at :1632-1639; no-op arm; zero call sites; server applies tangible rewards). **Material nuance**: the protocol has NO quest-state update message — after any server trigger the client's QuestState goes stale until relogin, and dialog scripts evaluate conditions against that stale state. **Wiring it up would NOT double-apply rewards** (verified: the client reward functions are explicitly server-aware — inventory/XP/money/teleport arms are deliberate no-ops; the client chain is a deterministic mirror of the server loop). Wire the arm (~5 lines) to restore the presentational quest-state mirror; Low priority (correctness, not perf). |
| F15 | CONFIRM (no action) | Accurate; `Query::single/single_mut` in Bevy 0.18 return `Result` (no panic) and all sites use `let Ok(...) else return false` — graceful degradation confirmed. Cost is tens of ns (cached QueryState iteration). Consolidating queries would INCREASE cost (fetches all components on every call). Confirm no-change. |
| F16 | CONFIRM (no action — info-only) | Accurate; the disabled-path cost is ~5–20 ns/instruction (log-crate gate + tracing bridge LevelFilter rejection — the gate isn't strictly the macro gate but the result is identical); the RUST_LOG=debug warning (10–100× slowdown) is accurate. Confirm info-only. |

**Cross-cutting note for the whole doc set:** the vendored source folder `bevy-collection\bevy-0.18.1` is actually **0.19.0-dev** (its `Cargo.toml` declares `version = "0.19.0-dev"`); the client builds against crates.io Bevy 0.18.1. API citations verified only against that folder must be re-checked against 0.18.1 before implementing. Additional note: the wip scripting change set (F1/F2/F3/F4/F6) was **read-audited but never compiled** — it must go through the standard build-check before merge, ideally with the F3 rewrite isolated from the F1/F2/F4 slice.
