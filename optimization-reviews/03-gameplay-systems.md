...
Independent sub-agent scrutiny of every finding (GC/PC/CM/SE/CD/EF/PR/DD/BL/RG) against the actual source, Bevy 0.18.1, and the previous implementation attempt on `wip/local-changes-2026-08-04` (regressions in `12-validation.md`). Verdicts per finding:

| Finding | Verdict | Scrutiny result / action |
|---|---|---|
| GC-01 | PARTIAL | Structure confirmed (100 match arms, not ~60; 67 `commands.queue` closures; double entity lookups). The event-driven refactor is high-effort/high-risk for zero perf gain (message processing is bounded) — **defer**. If ever done: per-message handler extraction preserving FIFO + closure guards, and write the missing `gameplay-core.md` arch doc first (the doc's own gap). Note the "defeats parallel-query safety" rationale is moot — closures can't be parallelized regardless. |
| GC-02 | PARTIAL | Count accurate (67 sites), but "outer system already has the components" is imprecise — the outer system has **zero queries** (16-param limit is already hit; adding a `Query` param won't compile). Direct queries are actually *safer* than closures (closure `world.entity_mut` panics on missing entity; `query.get` skips). Defer/fold into GC-01; if done incrementally: bundle the 10 writers into a `SystemParam` first, convert only the ~28 single-entity single-component arms. |
| GC-03 | PARTIAL | Claim accurate (uncapped drain; server bursts spawn floods). wip implemented caps 4096/64/64 (OK, FIFO-safe), but **4096 is toothless for the documented "hundreds of messages" burst** — lower the game cap to ~256–512 to actually smooth zone-join hitches; the 64/64 login/world caps are harmless no-ops. |
| GC-04 | CONFIRM | Accurate; also omits the two **highest-frequency** sites (`game_connection_system.rs:777` MoveEntity, `:830` AdjustPosition — ~10 Hz while moving). wip demoted all ~10 sites to `debug!` (OK). Recommendation: port wip + keep the `:815-816` entity-not-found warn at `warn!` per 09-F13. Residue: `particle_sequence_system.rs:349,398` still log `info!` per particle (not in scope). |
| PC-01 | PARTIAL | Code citation exact; impact overstated — the scan is event-driven (keypress), ~1–2 µs at realistic drop counts (120 s expiry, sector-limited). **"Skip when pickup in flight" is NOT safe**: `Command::PickupItem` is never cleared when the target despawns — re-pressing the key is the current recovery mechanism. Defer; if pursued, first fix the real bug (clear PickupItem on target despawn). |
| PC-02 | PARTIAL | Clone real (~1 ns, 40-byte memcpy). Doc errors: `PlayerCommandEvent` contains **no `ItemReference`** and has zero heap fields — "per-input alloc" is wrong. Option A (match by value) is impossible without the clone; Option B (separate hotbar-translation system) is the only clean path. **Reject** — saves nothing measurable. |
| PC-03 | PARTIAL | Duplication confirmed (exact line refs). The behavioral difference claim is **REFUTED**: both paths emit `ChatboxEvent::System("Invalid target")` — no silent drop exists. The real differences: skills self-fallback to self-cast with no selection; consumables require a selection; consumables do a redundant second client-entity query. Extract `resolve_skill_target(fallback_to_self: bool)` preserving both behaviors. |
| PC-04 | PARTIAL | 10 Hz is a floor, not a ceiling (direction changes send immediately, up to frame rate). "Chatbox warns" is wrong — it's structured-log spam. 5 Hz throttle is safe: the server's `Move` handler has **zero validation** (teleport checks exist only on the `MoveCollision` flight path, already 10 Hz); lead time 0.25 s > 0.2 s interval. Implement-with-changes: `WASD_MOVE_COMMAND_INTERVAL_SECS` 0.10 → 0.20 only; keep immediate sends + lead time; disconnected half already handled by wip log demotion. |
| CM-01 | CONFIRM | Accurate (severity closer to Medium: scan runs only for `is_kill` entries past the 0.25 s grace, short-circuits). The wip `ProjectileIndex` is correct: world-verification makes it bit-identical to the old scan in every ordering; combat-sync invariants untouched. §3.6's "leak" is **not currently manifest** (projectiles have exactly 2 despawn paths, both remove from the index; zone-change/reconnect/effect despawns don't touch projectiles). Implement as-is (merge wip); optional one-line hardening on reconnect. |
| CM-02 | CONFIRM | Accurate (note: cap 32 is `with_capacity(32)` — a soft bound, not a real cap). Swap-remove is provably safe (consumers verified order-independent; hit_event accumulates with `+=`/`|=`). **Defer** — purest micro-optimization in the doc; do only if `hit_event_system` is next touched anyway. |
| CM-03 | PARTIAL | Duplication confirmed — but the split is **load-bearing**: `pending_damage_system` is the grace/cap fallback, `hit_event_system` is the hit-frame trigger; consolidating kill application into pending-only would delay every normal in-range kill ~1.5 s (reintroduces the combat-sync desync). Also found a latent asymmetry: pending removes `ClientEntity` unconditionally, hit_event only for non-Characters. Correct fix: extract a shared `apply_kill_components` helper preserving exact per-path semantics (incl. the Character exception); keep both decision points; re-test one-hit/projectile/AoE kills. |
| CM-04 | CONFIRM | Accurate (7-component unconditional query; severity low, µs-level). Fix is simpler than proposed: a `Changed<PendingSkillEffectList>` filter is self-sustaining (the aging loop's own writes re-arm the tick). **CRITICAL trap**: the query is shared with the hit-frame effect-application path — filtering it would defer all skill effects to the 2 s expiry. Must split into two queries (event-driven path unfiltered; aging path filtered). Low priority; batch with SE-01. |
| CM-05 | CONFIRM | Accurate; `KILL_MAX_DAMAGE_AGE = 1.5` is a hard `\|\|` cap that survived wip unchanged — it bounds the F1 culling freeze interaction too. Keep-as-is (cap comment already exists). |
| SE-01 | CONFIRM | Accurate (doc has an internal contradiction on where the `get_status_effect` lookups sit — they're tick-gated; the summary line is right). Accumulator gate is feasible **only** if gate frames add the accumulated duration to each entity's `per_second_tick_counter` (a naive one-frame-delta gate kills poison entirely). Query slimming safe (`_ability_values`/`_mana_points` unused). Implement-with-changes. |
| SE-02 | CONFIRM | Accurate — and even better than claimed: the id-dedup guard short-circuits same-id refreshes, so churn is only on apply/expire of *changed* ids. Keep-as-is (reuse option adds complexity for ~zero gain). |
| CD-01/CD-02 | CONFIRM | Both no-action verdicts accurate: 4 allocation-free loops over a single-entity `Cooldowns`; delta-time decrement is frame-rate-independent with correct underflow guard. No change. |
| EF-01 | CONFIRM | Accurate (writers are hit/consumable/status-event driven). wip already demoted all 3 sites to `debug!` (warn kept). Implement as-is — a port of the wip change. |
| EF-02 | PARTIAL | Structure real, but "4 query gets per grandchild" is wrong (2 per grandchild + 1 per child), and it is **not** the dominant effect cost (per-frame storage-buffer churn in `particle_sequence_system` dwarfs it). Work-stack saves nothing (each leaf visited once; no recursion). Finish-state events don't cover infinite-loop sequences (auras) — disproportionate. **Defer**; the real wins are 02-F1/F2 and the per-particle info logs. |
| EF-03 | CONFIRM | No-action verdict accurate (lookups only on flagged hit frames; O(1)). Wording nit: `get_npc(npc.id)`, not `get_npc_motion`. F1 culling interaction (§3.4) is the only concern and is tracked there. |
| PR-01 | PARTIAL | Claim confirmed; wip in-place mutation correct (12-validation OK). **Reject the target-translation cache**: `projectile.target` is set once and never changes — caching would turn homing into straight-line flight and produce stale-target hit events. The 2 per-frame lookups are O(1) and trivially cheap. |
| PR-02 | PARTIAL | NaN mechanism real (glam `normalize` of zero = 0·∞), but the claimed consequence is **false**: the `complete` branch fires on the same first frame *before* the transform write, so the NaN never reaches the transform. Reclassify as defensive hardening; prefer the existing `normalize_or` helper over an early-out. (Separate real hazard: `move_speed == 0` → `total_time = inf` — data-dependent, out of scope.) |
| DD-01 | CONFIRM | Accurate and understated (material mutation also forces a per-entity mesh re-extract each frame). The wip implementation is a **NO-OP** (same size-0 `buffer_description.size` bug as 02-F1/F2 — the grow branch fires every frame). Corrected fix: `with_size`-created buffers + `set_data` in place + material mutation strictly guarded to handle changes. Option (b) shared static buffer is not viable with the current shader/binding design. |
| DD-02 | PARTIAL | Claim accurate (meshes identical; shader reads only `@builtin(vertex_index)`). **Critical trap**: the spawner's existing `Rectangle` mesh is 4-vertex and *indexed* — sharing it as-is renders only one digit quad. Must construct the same 60-vertex non-indexed POSITION-only mesh in `DamageDigitsSpawner::load`. Recommended variant: a pool of count-exact shared meshes (1..=10 + miss) that also folds in 02-F21's freebie. |
| DD-03 | PARTIAL | Mechanics confirmed, but "resources already hold handles" is wrong (the spawner has no `Assets<...>` access and 3 new `ResMut` params would thread through 4 call sites), and the parent entity is **unavoidable** (`transform_animation_system` overwrites the child transform absolutely). Net benefit ≈ 0 — **defer** until DD-02 lands (then it becomes a natural 20-line follow-up). |
| BL-01 | PARTIAL | The count scan runs once per frame, not per event; the `min_by` eviction runs per spawned spatter when at cap (worse granularity). Not the blood hot spot (µs at `max_spatters=100`). The fix is sound (verified: no despawn paths exist, uniform lifetime ⇒ insertion-order == oldest). **Defer**; prioritize BL-02/BL-04/BL-05. |
| BL-02 | CONFIRM | Claim accurate; the wip regression mechanism (§3.7) **confirmed** — the cached entity list misses freshly spawned parts (equip change) → blood disappears until next hit. The doc's own fix (a) (walk only on dirty frames) *is* the regression. Best fix (not in doc): read the authoritative part lists already maintained on the entity — `CharacterModel.model_parts` / `NpcModel.model_parts` — on clean frames instead of the walk or the cache; keep the dirty-frame walk for the rare entities with neither. |
| BL-03 | CONFIRM | Accurate (verified: no per-frame HP writer exists — only event-driven writes + 1 Hz poison ticks). wip `Changed<HealthPoints>` filter works (`Added ⊆ Changed` covers spawn). Minor: settings-slider threshold changes apply only after the next HP write; max-HP changes without HP writes delay visibility. Implement as-is (wip) + optional config-changed pass. |
| BL-04 | CONFIRM | Accurate (projection eagerly runs before the cap check; 2–3 per hit; cap 4). wip implemented the lazy-closure fix correctly — semantics preserved. Implement as-is (merge wip). |
| BL-05 | CONFIRM | Accurate (materials are per-spatter, not shared per decal — each spawn `add`s; re-prepare churn confirmed). Fix refinement: gate on **elapsed time (10 Hz)**, not alpha delta — alpha is constant for the first ~21 s while the wet→dry color keeps drifting. Optional epsilon gate on the written color. Implement-with-changes. |
| BL-06 | PARTIAL | Confirmed (`:110` count is dead code — always equals the `:77` count). Count-once half safe and **already on wip** — port it. **`Local<ThreadRng>` is infeasible**: `ThreadRng` is `!Send` with no `FromWorld`; `thread_rng()` is a TLS lookup + Rc bump (~ns, no allocation). Doc corrections: drop the Local<ThreadRng> suggestion; ":110 is dead code". |
| BL-07 | CONFIRM | Accurate (0.3 vs 1.5 vs 1.0; unused). **Remove, don't normalize**: default 1.5 is load-bearing for current visuals (normalizing without compensating counts/sizes would visibly change gameplay). Also remove the plugin stub constructors (`blood_effect_plugin.rs:82-90`) and fix stale doc comments. Implement-with-changes (removal). |
| BL-08 | CONFIRM | Accurate — bias is **exactly zero** with 8 variants (power of 2). Leave-as-is; fix with `gen_range` if the file is touched (same pattern at `gash_wound_system.rs:226`). |
| RG-01 | CONFIRM | Gating inconsistency confirmed, but the doc's fix sketch is **over-broad**: registration block 2 also contains environment systems (`world_time_system`, `zone_time_system`, starry-sky, terrain lighting) that must NOT be gated (zone_time_system drives the menu sky and viewer lighting). Gate per-system on the ~16 combat/effect systems only (wip already gated projectile/spawn_effect); leakage is currently harmless (Command entities only exist in Game). |
| RG-02 | CONFIRM | Accurate; verified no entity carries the blood components outside Game (character select spawns no HP/AbilityValues entities). Gate all 4 plugins (8 systems) + the two DirtDashPlugin systems. Startup atlas systems stay ungated. |
