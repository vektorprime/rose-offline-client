...
Independent sub-agent scrutiny of every finding (F1–F18) against the actual source, oddio 0.6.2 / lewton (vendored), the game data, and the previous implementation attempt on `wip/local-changes-2026-08-04` (regressions in `12-validation.md` §3.10/§3.11). Verdicts per finding:

| Finding | Verdict | Scrutiny result / action |
|---|---|---|
| F1 | PARTIAL (implement-with-changes) | Structural claims accurate (no load-time decode; per-instance decode in `Last`; ring sizes 125 ms/500 ms; lewton ~0.2–1 ms/packet). **The burst profile is WRONG**: fill() runs every frame with only ~1 packet of ring space per frame — steady state is a constant ~1-packet/frame/stream tax, not periodic spikes. Fix (a) pre-decode short/repeating OGGs — sound, low-risk (mirror `wav.rs`; off-thread in the loader; gate on **compressed size** ~<1.5 MB, not duration — the loader can't know "repeating"; 3-min BGM stays streaming), NOT implemented anywhere. Fix (b) chunked refill (50 ms budget) — **already on wip** (as "08 F9"), contract-correct, 3× headroom at 60 fps (12-validation §3.11's "zero slack" math is wrong; underruns need <20 fps); it bounds spawn bursts only. Fix (c) background decode thread — defer (low ROI after (a)+(b)). Include the F16 stereo→mono downmix for pre-decoded spatial files. |
| F2 | CONFIRM (fix on wip, needs rework before merge) | All three sub-claims verified (fill every sound every frame; queue sorts but never rejects; oddio 1/r with no cutoff; -31 dB at 150 m radius-4 confirmed). Wip implements the queue gate (AUDIBLE_CUTOFF=100), hysteresis (resume 80 m), cull-before-allocation, MAX_BUFFERED_DISTANCE=150 — **but with the two §3.10 MED regressions, both real**: one-shots hard-despawned mid-play (small pop at -28 dB; worse for large radii), and loops `stop()`+restart (phase jump + ring re-alloc churn per hysteresis crossing). **New finding**: the fixed cutoff is **radius-invariant** — IFO sound objects carry `range` (up to 25.5 m radius) and would be audibly cut at 100 m. Required rework: don't cull started one-shots (F3's 64-cap bounds them); for loops, gain-ramp→`pause()` (no phase jump, ring retained), resume with ramp; make the cutoff radius-aware (`distance > cutoff + radius`). Doc correction: the `spatial.rs:418` debug_assert is about callback duration, not distance (the real clamp at :431-433 is graceful). |
| F3 | CONFIRM (fix on wip, safer than the sketch — merge) | Cap is a per-frame spawn budget (3/frame → 180/s → ~54–180 concurrent by Little's law); `max_mix_count` is populated from STB_SOUND but unused. The wip fix is **safer than the review's sketch**: `MonsterSound` marker component + query count (`MAX_ACTIVE_MONSTER_SOUNDS = 64`) — the "lost increment starves all monster sounds" risk is structurally eliminated (every despawn path decrements implicitly with the entity); monster sounds are always one-shots, so the flagged failed-load repeating path can't leak. **Not on main** — merge. Optional per-id `max_mix_count`: defer (bounded by cap 64 anyway). |
| F4 | PARTIAL (keep fix (a) on wip; reject fix (b) as written) | Ring math verified: 500 m → 274.8 KB. **Fix-sketch numbers wrong**: 150 m = 94.8 KB (2.9×, not 5×/55 KB — the +0.1 s buffer term was dropped). Fix (a) `MAX_BUFFERED_DISTANCE=150` is on wip and safe (clamp is graceful; 150 > cutoff 100 with margin). **Fix (b) Frames path REJECTED as written**: `SpatialSceneControl::play` requires `S: Seek` and `Gain<T>` has no `Seek` impl (only `FixedGain`) — it doesn't compile; and `FixedGain` has no `Controlled` impl → volume sliders/mute would stop affecting all one-shot WAVs (a real regression); `FramesSignal` doesn't loop (repeating sounds need `Cycle<Frames>`); end-detection needs polling with up to 0.29 s late despawns. Keep (a); defer (b) indefinitely. |
| F5 | PARTIAL (defer) | Mechanics confirmed (2 sqrts + div + one atomic swap per sound per frame — ~50–100 ns/sound, ~5–10 µs/frame at 100 sounds; the "100+ RefCell borrows" is inaccurate — the RefCell is on the audio thread, not the producer side). 10–20 Hz throttle is SAFE (oddio's 0.5 s position smoothing exists precisely for discrete updates); epsilon-skip must gate on **both** source and listener delta (velocity is listener-relative); **HIGH risk: never gate `fill_mono`** (ring drains every frame) and the velocity divisor must be the throttle interval, not `time.delta_secs()`. NOT implemented (wip has F2/F4/F15 only). Defer until F2/F3/F4 are validated in-game. |
| F6 | CONFIRM (gate already on wip; const hoist won't compile as written) | Accurate (terrain+tile lookups before any distance filter; ~1–2 footstep events/s per walking NPC; ~30–60 ns/event). Distance gate safe (80–100 m; footsteps are one-shots — no hysteresis risk; prefer the shared `AUDIBLE_CUTOFF`). **`const DEFAULT_STEP_SOUND: SoundId = SoundId::new(653)` will NOT compile** — `SoundId::new` is a non-const fn; use `const DEFAULT_STEP_SOUND_ID: u16 = 653`. 653 verified against the original C++ client (`CHAR_STEP_SOLID`). The gate + hoisted `get_sound(653)` are **already on wip** (not recorded in 12-validation — add the row; §3.10's hard-cut risk doesn't apply to one-shots). |
| F7 | CONFIRM (true crossfade already on wip — merge) | All claims accurate (frame-counted `+= 16` → 1–4 s depending on fps; no gain writes during FadingOut; new track after despawn). **Bonus bug found**: zone change mid-fade orphans the fading track — plays forever at full volume (the F7 fix repairs this too). The wip implementation is correct: `time.delta_secs()`, immediate new-track spawn, per-frame `SoundGain::Ratio` ramp → gain_changed system → `set_amplitude_ratio` (oddio smooths over 100 ms — no click), zone-change branch despawns the old entity. Merge; it also resolves F14. |
| F8 | PARTIAL (implement as hygiene) | Claim accurate (RwLock + handle clone per load; all callers main-thread — effect_loader does NOT use the cache, direct loads only). **The OOB-miss "waste" is practically unreachable**: `SoundData.id` is the STB row index, the cache is sized to `sounds.len()`, and every call site resolves through `get_sound` first — ids are in-range by construction (the fallback is defensive dead code; Bevy dedups by path anyway). Fix safe: plain `Vec<Option<Handle<AudioSource>>>` in a Resource + `debug_assert!` in set + keep the OOB fallback; 5 callers flip Res→ResMut (all event-driven, negligible). Frame it as lock-removal hygiene, not a miss-path fix. NOT implemented anywhere — the only fully-unimplemented audio finding. |
| F9 | PARTIAL (fold into F1; already on wip) | Fresh-Vec-per-packet is **lewton-internal** (4+ allocs/packet inside the decoder) — a client-side fix can't remove them, only stop discarding one on the partial-write path; the drain/extend cost fires only ~2–8×/s per stream (the "thousands per second" is 1–2 orders overstated). The cursor+swap fix is **already on wip** (12-validation:86 OK) with a redundant `packet.drain` to drop and §3.11's budget fix to apply. Post-F1(a), `Streaming` handles only BGM + long one-shots — F9's standalone value collapses; fold in. |
| F10 | PARTIAL (fold into F1(a); mechanism overstated, real bug found) | `seek_absgp_pg(0)` short-circuits (no bisect; headers rejected with 1-bit checks) — real cost ~1–5 µs per loop, no stutter risk; "re-parse headers" overstates. **The real per-loop cost the doc missed**: after the window reset, the first audio packet decodes to 0 samples and is skipped — every loop boundary drops ~1 packet (~23–93 ms of audio); the Buffered path is sample-exact. Pre-decode = the SAME change as F1(a); threshold = compressed file size (≤256–512 KB covers short loops, excludes BGM), not duration. Not on wip — implement with F1(a). |
| F11 | CONFIRM (implement-with-changes) | Mechanism verified; "all played WAVs" slightly overstates (effect/ambient/vehicle/BGM sounds hold entity-only handles and already evict — the cache-routed majority leaks). Memory math plausible (20–100 MB typical sessions; hundreds of MB at near-full DB). Playback data is Arc-cloned at start — **eviction by handle-drop is safe**; the "queued-but-not-started" concern can't happen via that path (entities hold strong handles that block eviction). **Hazard found**: never use `Assets::remove` for eviction — the load state survives (no drop event) while the asset is gone → entities stuck forever (despawn only on Failed/NotLoaded). Implement: call `SoundCache::clear()` on zone change or handle-drop LRU. Not on wip. |
| F12 | CONFIRM (implement-with-changes — UX fix) | Accurate (4 effect spawn sites + ambient objects: `SoundGain::default()` 1.0, no `SoundCategory`); even the master mute can't silence them (no category → the settings apply-loop can't touch them — a stronger UX bug than stated). Doc imprecision: the gain_changed systems match `Changed<SoundGain>`; it's the settings apply-loop that keys on `SoundCategory`. Fix: add `Effects` + `Ambient` category variants (defaults **1.0** — conscious choice; aliasing to `other_combat` 0.5 would halve ambience), thread through `spawn_effect` (7 call sites) + objects.rs; add UI sliders. Not on wip; no conflicts. |
| F13 | CONFIRMED (implement-with-changes) | Writes + chain verified (Bevy `Mut::deref_mut` always flags Changed). **Nuance**: creak's gain genuinely changes every frame (wave-driven via boat_buoyancy) — the guard saves nothing there; it saves for wind/flap at steady state. Category claim confirmed ("correct only by accident"). Fix: compare-guard all three writes; **do NOT change the category in this fix** (no Vehicle variant exists; adding one is scope creep for a trivial-CPU finding). Not implemented. |
| F14 | CONFIRM (trivial delete; moot if merging wip) | Param truly dead; the scheduler impact is "none today" (no other Update-schedule system queries GlobalSound — the global systems are in `Last`). Fix: delete the param + clean the `Query` import. **The wip F7 rewrite already replaced it** with a used `Query<&mut SoundGain>` — moot on the wip tree; apply to main only if F7 isn't merged alongside. |
| F15 | PARTIAL (fix on wip — implement as-is) | Mechanisms confirmed (glam `normalize` = 0·∞ → NaN; the glam_assert is a no-op with default features — silent). **Frequency overstated**: the camera sits off-player on a dolly arm; exact zero needs camera-collision clamping the arm to 0.0 or rig-init frames — rare, but nasty (NaN propagates through oddio's spatial mixer with zero guards). `delta == 0` is near-unreachable (no pause usage) but correct hygiene. Fix (`try_normalize().unwrap_or(Vec3::Z)` + `delta.max(1e-6)`) is **already on wip** (not recorded in 12-validation — add the row). Adjacent: the same unguarded `normalize()` exists at orbit_camera_system.rs:211. |
| F16 | PARTIAL (defer; doc direction error + data-verified latent) | Mechanism CONFIRMED (spatial graph is hard-typed mono; fill_mono writes interleaved stereo). **The claimed effect direction is WRONG**: a stereo source through a mono stream plays at **0.5× speed/pitch (2× duration)** — the data volume is double, not the rate. **Never triggers with the shipped data**: 295 of 296 WAVs mono (the stereo one, Quest_E, is UI/global-only); all 19 OGGs are BGM (global path, channel-correct); FILE_SOUND.STB has zero OGG refs; all spatial-fed files are mono — a latent bug. Fix (if ever): per-fill downmix in `fill_mono` covering both variants; **decode-time downmix rejected** (shared asset would collapse global stereo BGM to mono). Correct the doc. |
| F17 | PARTIAL (implement-with-changes) | Path rebuild confirmed (event-driven, transitions only; Bevy dedups); the `unwrap` is confirmed-latent-unreachable (Vehicle + model are atomic) but a live hazard for the `:54` model-swap TODO. **Discovery**: `vehicle_sound_system` is registered **twice** (lib.rs:1216 chained + :1220 standalone) — two instances per frame (harmless via the state guard, but doubles query work). Fix: route both loads through SoundCache; replace unwrap with let-else; flag the duplicate registration separately (behavior-affecting). Not implemented. |
| F18 | CONFIRM (no action — informational) | Verified end-to-end against oddio: the mixer holds a strong Arc, `handle_dropped()` only closes the ring, removal waits for `is_finished()` (ring fully drained) — no audible truncation; tails ≤125 ms spatial / ≤500 ms global; the `stop()`-on-despawn hazard is real and the client correctly avoids it. **No change required — keep as documentation** (the note prevents a future regression). |

**Cross-cutting note for the whole doc set:** the vendored source folder `bevy-collection\bevy-0.18.1` is actually **0.19.0-dev** (its `Cargo.toml` declares `version = "0.19.0-dev"`); the client builds against crates.io Bevy 0.18.1. API citations verified only against that folder must be re-checked against 0.18.1 before implementing. Also note: **all 08 audio fixes live only on the wip branch** (F2/F3/F4/F7/F9 + F15 guards + F1(b)); the current main/working tree has none of them.
