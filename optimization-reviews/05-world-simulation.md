...
Independent sub-agent scrutiny of every finding (F1–F15) against the actual source, Bevy 0.18.1, and the previous implementation attempt on `wip/local-changes-2026-08-04` (regressions in `12-validation.md`). Verdicts per finding:

| Finding | Verdict | Scrutiny result / action |
|---|---|---|
| F1 | CONFIRM | Claim accurate (per-part rotation rewrites: 5–6 `sin` + 2–3 quats per vegetation part per frame, hundreds–low thousands per zone, no culling). **Implemented on wip** (distance culling in `wind_effect.rs`) and validated OK — merge. Tier-4 GPU sway remains deferred. |
| F2 | CONFIRM | Claim accurate (300 max birds, 3 lookups + ~6 trig + 3 writes each, no cull). **Implemented on wip** (150 m cull), validated OK — merge. |
| F3 | CONFIRM | Claim accurate (2,000 entity particles, per-frame CPU billboards, 2,000 alpha draw calls; UI slider allows 20,000). **Implemented on wip** (clamp 1000 + >150 m / behind-camera billboard skip), validated OK with §3.8 residuals: (a) the clamp is silent while the UI still advertises 20,000 — align the slider with the cap (settings decision); (b) no hysteresis at the 150 m cull boundary — add skip >150 m / resume <100 m. 10 Hz billboard quantization would be a visible regression — do not. |
| F4 | CONFIRM | Claim accurate (unconditional `ZoneLighting`/`ZoneTime` writes every frame defeat all `is_changed` guards). The wip per-field epsilon guards (1e-3 on `state_percent_complete`, 1e-4 on colors, exact `!=` on time) are correct; quantization steps every ~0.5–2.6 s are imperceptible; the starry-sky twinkle (`time`) still writes per frame as required. **The doc's "early-return on `!world_time.is_changed()`" variant would NOT work** — `world_time_system` writes the accumulator every frame, so the resource is always changed (the exact trap doc risk #7 flags). Merge the wip per-field form. |
| F5 | CONFIRM | Claim accurate (guard always true → byte-identical sun rotation written ~599 of every 600 frames → light/cascade re-validation churn). Wip value-diff guard correct (rotation is deterministic per 10 s tick). Merge. |
| F6 | CONFIRM | Claim accurate, one imprecision: during Morning/Evening lerps the colors genuinely change **every frame** (33% of the day), not "a few times per second" — the value-diff is the load-bearing half. Wip value-diff-only is correct. **Do NOT convert to a `(state, settings)` latch — it would freeze the entire dusk/dawn color ramp.** Merge wip as-is. |
| F7 | CONFIRM (claim) / fix has confirmed gap | Claim accurate (identical shadow/illuminance writes every frame; no latch). The wip `(ZoneTimeState, bool)` latch has a **confirmed gap** (extends 02-F13): `apply_shadow_quality_system` runs on *any* `GraphicsSettings` change (not just presets) and writes `shadows_enabled = true` on **all** directional lights; the latch key can't see that write, so during Evening/Night a brightness/quality change leaves the **moon casting shadows in every state** and the **sun shadow map rendering every frame at night** (self-heals only at the next state transition). Correct fix: replace the latch with per-field **value-diff writes** (same pattern as F5/F6) — O(1)/frame, re-asserts correct values in the same frame after any external clobber. |
| F8 | CONFIRM | Claim accurate (3 allocs + sort temp = 4; sweep runs over the whole zone before the cull). **The exact fix already exists on wip** (cull-inside-collect + `Local` buffer reuse) and was reviewed in 12-validation §3.13 — it was **lost in the code-simplification cleanup** on main. Re-apply the wip version verbatim (~50 lines; the §3.13 `near_idx` "fragile invariant" note is defense-in-depth only — both loops share one query with an identical predicate). Note: `monster_separation_system` did NOT copy this pattern (it's still plain O(n²)) — 04-F6 is separate. |
| F9 | CONFIRM | Claim accurate (view bind group recreated + `WorldUiBatch` per rect per frame). **Implemented on wip** (BufferId-keyed cache), validated OK — merge. Two additional findings: (a) the doc's "batch entities are despawned by the ECS between frames" is **false** — nothing despawns them; render-world batch entities **accumulate unboundedly** (real leak, must be fixed with a despawn/pool); (b) the working tree is missing the wip's `TextureFormat::bevy_default()` fix (12-validation:28, required for reflection-camera world-UI) — restore it. Diff-based rebuild was rejected: the data changes every frame by design. |
| F10 | CONFIRM | Claim accurate (all healthbar rects rewritten per frame). **Implemented on wip** with a `last_health_percent` latch — and the wip's choice matters: a plain `Changed<HealthPoints>` filter is **unsafe here** (name tags spawn several frames after their entity; the filter could skip initialization). Merge wip as-is. |
| F11 | PARTIAL | Color rewrite confirmed; the doc's "offsets/positions need per-frame updates" is **REFUTED** — bubbles are children of their target; position follows via hierarchy; no per-frame position writes exist at all (only the expiry-timer decrement is mandatory). The orphan sweep is effectively **dead code** (Bevy 0.18 despawn is recursive; a bubble cannot outlive its target; the existing `RemovedComponents<ClientEntityName>` system covers the component case). Fix (if hygiene is wanted): color gate using the *same* `total_time * fade_start_fraction` formula as `get_fade_alpha` (behaviorally identical) + empty-sweep early-return; **reject** the `RemovedComponents<ChatBubble>` trigger (fires on bubble despawn, not target despawn — semantically wrong). Defer — sub-microsecond at current bubble counts. |
| F12 | CONFIRM | Claim accurate (per-frame timer decrement over all NPCs; fresh phrase `Vec` per trigger). Fix safe and trivial: pre-flatten phrase lists once + `gen_range` + 4 Hz cadence for the decrement. Not implemented; low priority. |
| F13 | CONFIRM (no action) | Verified: `thread_rng` once per run, per-NPC early-outs, ≤1 roll per loop, capped at 3 concurrent sounds + 4 m radius. Reconciles with 01-F16 (the system is not read-only — one `Option<usize>` write per NPC per frame — but that's negligible; 01-F16's mitigations were unsound). **No change required — remove this finding from the doc.** |
| F14 | PARTIAL (no change required) | Mechanics real (per-frame resource writes) but: the "churns WindState extraction" claim is **false** (no `ExtractResource` on `WindState`); the "blocks F1 tier-3 gating" justification is invalid (tier 3 is time-slicing, not change-detection); an epsilon guard on `WindState` is infeasible (the noise clock must advance every frame; sailing reads wind per frame for thrust/sail/audio). The only defensible micro-fix is a `!=` guard on `global_intensity` in `sync_vegetation_wind_system` (constant 0.3 in land zones → ~0 Hz writes, zero behavior change) — pure hygiene. **No change required today — remove this finding from the doc.** |
| F15 | CONFIRM (no action) | All listed pieces verified already-correct (state-transition-only night-effect hiding, starry-sky write-if-different, camera `completed()` gate, event-driven login/select flows). **No change required — remove this finding from the doc.** |

**Note (per 2026-08-04 review):** F13, F14, F15 require no changes — verified no-action; retained in the document for the record (their substance is summarized in this verification section).
