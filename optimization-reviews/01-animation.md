...
Independent sub-agent scrutiny of every finding (F1–F18) against the actual source, Bevy 0.18.1, and the previous implementation attempt on `wip/local-changes-2026-08-04` (whose regressions are documented in `12-validation.md`). Verdicts per finding:

| Finding | Verdict | Scrutiny result / action |
|---|---|---|
| F1 | PARTIAL | Claim real, but the fix as written is a **NO-OP** for the main population: character/NPC root entities have no `Aabb`, so Bevy frustum culling never sets their `ViewVisibility` to HIDDEN — `should_animate_entity` always returns true and the distance check never runs (this is why the wip culling "silently did nothing"). Regression risks are real *if* culling is made effective: skipping `advance()` keeps `completed()` false → up to 1.5 s delayed kills (`hit_frame_expected`), command-state stalls (command_system blocks on uncompleted Attack/Die motions), and an effect-despawn leak (culled effect children never `completed()`). Correct fix: **always advance `AnimationState`; cull only the bone-write/material-update work**, gate on distance (with hysteresis, e.g. skip >260 m / resume <200 m) since root `ViewVisibility` is unusable, never cull combat-relevant entities, and add a debug counter to verify culling fires. |
| F2 | PARTIAL | Claim mostly accurate. Doc errors to correct: (a) "visibility propagation also re-walks bones" is **false** in 0.18.1 — visibility propagation is `Changed<Visibility>`-driven and bones never change after spawn; (b) "scales quadratically" is imprecise (dirty-tree walk is per-root, bitset-deduplicated); (c) the guard's "~fps/animation_fps frames skipped" expectation is **wrong** — `current_frame_fract` is time-continuous, so the sampled pose changes every rendered frame; guards only suppress near-static bones and slerp numeric churn. **Reject the `iter_many_mut` batching**: identical per-entity lookup cost, and the "zip with joints" sketch silently misaligns all subsequent pairs if any joint is skipped (items carry no `Entity`). Keep the epsilon write guards (1e-5/1e-4) — behavior-preserving. |
| F3 | PARTIAL | Speed-guard confirmed and behavior-preserving (no `Changed<SkeletalAnimation>` consumers exist; `animation_speed` only read in `advance()`). The `Handle::default()` re-insert mechanism is **REFUTED**: all 18 call sites pass strong-gated getters or `asset_server.load` results, so a default handle can never reach the insert path — the `is_strong()` guard is defensive only. The real per-frame insert loop is the **failed-ZMO-load** path (strong handle whose asset fails to load → marked completed → re-inserted next frame), which `is_strong()` does not fix; optionally gate that on `get_load_state == Failed` later. |
| F4 | CONFIRM | Accurate (`Assets::get_mut` marks Modified even on value-identical writes; non-bindless path re-prepares bind group + uniform buffer per frame). Fixes #1 (precomputed flags) and #2 (early-out) correct as implemented on wip. Doc error: "~half the frames can be skipped" is wrong — `next_weight` advances every frame; the early-out fires only on start-delay / speed 0 / completed. Defer fix #3 (per-entity uniform): Bevy's extended-material pipeline cannot do per-entity uniform offsets without a custom render path. |
| F5 | PARTIAL | Mechanism confirmed (mesh mutation → full VBO/IBO re-upload per frame), but the impact estimate is **~100× overstated**: sails have 25–81 vertices (not 2–8k); 38 sails fleet-wide ≈ 46–156 KB/frame. No BVH/tangent re-preparation (sails have none). The 15 Hz quantized fix works with no aliasing (luff ω=1.27 Hz ≪ 7.5 Hz Nyquist; max 0.16 m step, invisible when sail filled). Shader-deformation path not justified — skip. |
| F6 | PARTIAL | Core premise false: `BlinkClipPlugin` is **never added to the app** — `sync_blink_clip_to_state` (and the dead twin) have never executed; runtime cost is zero. Reclassify as dead-code removal: delete the plugin, `BlinkClipState`, `BlinkUniform`; keep `BlinkClip` + the blink-timer writer. Doc error: "value-equal inserts are no-ops in the render world" is wrong in 0.18.1 (`try_insert_batch` → `Column::replace` always overwrites + bumps the tick). |
| F7 | CONFIRM | Accurate, and worse: nothing ever removes IBP assets (`remove`/`remove_unused` grep = 0), so this is a session-long leak of one asset per cumulative spawn. Fix works; key the cache by the skeleton's **VFS file path** (or namespaced enum — a raw u32 gender/id key collides across characters/NPCs/vehicles and breaks skinning), stored in `ModelLoader` via `Mutex<HashMap>` (or switch spawn methods to `&mut self`). Sharing one handle across entities is the intended Bevy design (asset is zipped against joints in order). |
| F8 | PARTIAL | Duplication confirmed; `blink_state` removal (fix 2) safe — binding 105 is not even declared in `rose_object_extension.wgsl`. **Fix 1 (material cache) is NOT safe as written**: `blood_overlay_system` mutates `blood_overlay_texture`/`blood_params` per material per entity (default-on feature) — a shared material would paint blood on every sharer; also `AlphaMode` lacks `Hash` in 0.18.1. Cache only after blood goes copy-on-write (clone-on-first-blood), or reject and keep per-instance materials. Do NOT share `RoseEffectExtension` materials (F4). |
| F9 | PARTIAL | Accurate, **understated**: `VirtualFilesystem::read_file` has no cache, so each spawn also pays archive re-open + decompress, not just parse. `skeleton_index` (u16) maps 1:1 to a stable path — valid key. The wip implementation keys by `npc_id` instead: covers only same-id respawns (misses the common "50 NPCs share 8 skeletons" case) and retains N duplicate parsed copies. Re-key by `skeleton_index`. |
| F10 | PARTIAL | Accurate (memory only; magnitude unverifiable without game data). The doc's fixes are unworkable: the loader cannot distinguish skeleton vs object ZMOs (both via `ZmoAssetLoader`), and a consumer-set flag would mutate a shared path-cached asset (first consumer wins). Better fix: restrict scale storage to channel slot 0 — the only slot ever consumed (`sample_scale` callers hardcode channel 0). **DEFER** (≤ a few MB vs hundreds of MB of textures). |
| F11 | PARTIAL | Accurate, with two corrections: the window re-opens on every equipment change/NPC respawn, and a part mesh that **fails to load** never gets an `Aabb` → the entity polls forever with up to 9 `info!` lines/frame (5 sites, not 3) — that is the dominant real cost. Throttle via `Local<u32>` K=30 + `if query.is_empty() { return; }` is feasible and safe; AssetEvent-driven completion correctly judged fiddly — reject. Log removal safe (demote-to-debug preferred; the "Created collider" line is the only positive trace). |
| F12 | PARTIAL | "Global mutex-point" is inaccurate: Bevy's Main schedule is single-threaded and Update completes before PostUpdate, so `ResMut<Assets<...>>` in PostUpdate serializes against nothing (the only PostUpdate co-holder runs on `Changed<MoveMode>`). Handle clones are refcount bumps, not allocations. Only viable piece: borrow instead of clone (both handles). The read-only outcome depends on unimplemented F4 fix #3 — fold into F4. |
| F13 | CONFIRM | Accurate. Removal is safe and compiler-enforced: `AsBindGroup` binding indices are explicit (`#[uniform(100)]` etc.), so removing the field removes only binding 105 (already undeclared in the shader); no renumbering. Doc typos: `BlinkUniformBuffer` does not exist (only `BlinkUniform`). Note: removal is necessary-but-not-sufficient for F8 — blood overlay still blocks material sharing. Blinking's real home is the `BlinkClip`/`BlinkClipState` path (currently unrendered). |
| F14 | CONFIRM | Accurate. Fix sketch correction: `bevy_asset::io::Bytes` **does not exist** in 0.18.1 — the custom `ArcReader` (AsyncRead + AsyncSeek + `read_to_end` override) is the only route; must keep `use_cache` semantics (safe: cache never mutated in place). Identical to 07 F12 — implement once, not twice. LOW priority, defer. |
| F15 | CONFIRM | Accurate; the wip implementation is behavior-preserving (exact `!=` guards; `completed` flip and loop-count changes still written; no `Changed<SkeletalAnimation>` consumers exist). Doc overstatement: "makes `Changed<SkeletalAnimation>` meaningful" is only partially true — `current_frame_fract` still writes every frame, so the change tick continues in the common case; the win is reduced field writes. Implement as-is. |
| F16 | PARTIAL | Claim overstated: the system is not read-only (writes `NpcIdleSoundState.last_idle_loop_count` every frame for non-stopped NPCs) and the per-frame cost is LOW (bounded by `MAX_CONCURRENT_MONSTER_SOUNDS = 3` + 4 m sound radius). Both suggested mitigations are unsound: a visibility filter would silence audible-but-off-frustum NPCs, and a 0.5 s timer adds a missed-sound window. Doc 05 F13's "already optimal, leave as-is" is the correct conclusion. |
| F17 | CONFIRM | Accurate; pure dead-code removal (4 commented-out blocks + the `should_log` computation). Doc under-lists: also remove the unused `anim_count`/`completed_count`/`no_skinned_mesh_count` counters and the rotation block at 135–139. The 5 ZMO-texture-loader info logs are load-time only (may stay or demote). |
| F18 | CONFIRM | Accurate; no cost. Leave as-is — `WingMaterialPlugin` nests inside `WingSpawnPlugin::build`, so deleting the stub would orphan active wing scaffolding. Optional trivial cleanup: `run_if(on_event::<FlightToggleEvent>())` — not worth the churn. |

**Cross-cutting note for the whole doc set:** the vendored source folder `bevy-collection\bevy-0.18.1` is actually **0.19.0-dev** (its `Cargo.toml` declares `version = "0.19.0-dev"`); the client builds against crates.io Bevy 0.18.1. API citations verified only against that folder (e.g. F14's `Bytes`, 02-F1's `resize`/`data_len`) must be re-checked against 0.18.1 before implementing.
